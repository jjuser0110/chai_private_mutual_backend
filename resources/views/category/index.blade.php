        ...
        <div class="col-lg-12 mb-3">
            <section class="card">
                <div class="card-header" style="text-align: right;">
                    <a class="btn btn-xs btn-square btn-primary" href="{{route('category.create')}}">Create</a>
                </div>
                <div class="card-body">
                        ...
                        <tbody>
                            @foreach($category as $s)
                                <tr>
                                    ...
                                    <td>{{$s->password??''}}</td>
                                    <td><img style="width:60px; height:auto" src="{{asset('storage/'.$s->icon_path)}}"></td>
                                    <td><?php echo $s->is_active == 1?"<span style='color:#1bb500'>Active</span>":"<span style='color:red'>Inactive</span>" ?></td>
                                    <td>
                                        <a href="{{ route('category.edit',$s) }}" title="Edit"><i class="bx bx-edit-alt"></i></a>
                                        <a onclick="if(confirm('Are you sure you want to delete?')){window.location.href='{{ route('category.destroy',$s) }}'}" title = "Delete" style="cursor:pointer"><i class="bx bx-trash"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
