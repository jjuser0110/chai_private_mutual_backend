        ...
        <div class="col-lg-12 mb-3">
            <section class="card">
                <div class="card-header" style="text-align: right;">
                    <!-- <a class="btn btn-xs btn-square btn-primary" href="{{route('order.create')}}">Create</a> -->
                </div>
                <div class="card-body">
                        ...
                        <tbody>
                            @foreach($order as $s)
                                <tr>
                                    <td>{{$s->order_no??''}}</td>
                                    <td>{{$s->user->name??''}}</td>
                                    <td>{{$s->shop_item->item_name??''}}</td>
                                    <td>{{$s->status??''}}</td>
                                    <td>
                                        <a href="{{ route('order.edit',$s) }}" title="Edit"><i class="bx bx-edit-alt"></i></a>
                                        <!-- <a onclick="if(confirm('Are you sure you want to delete?')){window.location.href='{{ route('order.destroy',$s) }}'}" title = "Delete" style="cursor:pointer"><i class="bx bx-trash"></i></a> -->
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
